...
ax.plot(df['ds'], df["y"], marker='o', linestyle='-', color='blue', label='Actual')
ax.plot(forecast["ds"], forecast["yhat"], linestyle='--', color='red', label='Predicted')
...
merged = df.merge(forecast, on='ds', how='left')
merged['residual'] = merged['y'] - merged['yhat']

# TODO: residual 시계열을 시각화하세요.
fig4, ax2 = plt.subplots(figsize=(14, 4))

ax2.plot(merged["ds"], merged["residual"], marker='o', linestyle='-', label='Residual')
...
st.table(merged['residual'].describe())
